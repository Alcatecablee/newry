...
export async function statusCommand(options: StatusOptions) {
  ...
  try {
    ...
    if (hasConfig) {
      console.log(`${chalk.green('✓')} Configuration file found: .neurolint.json`);
      
      const config = await loadConfig();
      console.log(`${chalk.blue('  API URL:')} ${config.api.url}`);
      console.log(`${chalk.blue('  Enabled Layers:')} ${config.layers.enabled.join(', ')}`);
      console.log(`${chalk.blue('  Output Format:')} ${config.output.format}`);
      
      if (config.apiKey) {
        console.log(`${chalk.green('✓')} API key configured`);
      } else {
        console.log(`${chalk.yellow('⚠')} No API key configured (run: neurolint login)`);
      }
    } else {
      console.log(`${chalk.yellow('⚠')} No configuration found (run: neurolint init)`);
    }
    ...
    const jsFiles = await glob('**/*.{js,jsx}', { ignore: ['node_modules/**', 'dist/**', 'build/**'] });
    const tsFiles = await glob('**/*.{ts,tsx}', { ignore: ['node_modules/**', 'dist/**', 'build/**'] });
    
    console.log(chalk.white('\nFile Statistics:'));
    console.log(`${chalk.blue('  JavaScript files:')} ${jsFiles.length}`);
    console.log(`${chalk.blue('  TypeScript files:')} ${tsFiles.length}`);
    console.log(`${chalk.blue('  Total files:')} ${jsFiles.length + tsFiles.length}`);
    ...
    if (options.detailed) {
      ...
      console.log(chalk.white('\nCommon Patterns:'));
      const componentFiles = [...jsFiles, ...tsFiles].filter(f => 
        f.includes('component') || f.includes('Component') || /\/[A-Z]/.test(f)
      );
      console.log(`${chalk.blue('  Component files:')} ${componentFiles.length}`);
      
      const testFiles = [...jsFiles, ...tsFiles].filter(f => 
    ...
    console.log(chalk.white('\nRecommendations:'));
    if (!hasConfig) {
      console.log(`${chalk.yellow('•')} Run ${chalk.white('neurolint init')} to set up configuration`);
    }
    
    const config = await loadConfig();
    if (!config.apiKey) {
      console.log(`${chalk.yellow('•')} Run ${chalk.white('neurolint login')} to authenticate`);
    }
    
    if (jsFiles.length + tsFiles.length > 0) {
      console.log(`${chalk.yellow('•')} Run ${chalk.white('neurolint analyze')} to check your code`);
    }
