  ...
  results.forEach((result, index) => {
    ...
    if (result.success && result.layers) {
      result.layers.forEach(layer => {
        const layerStatus = layer.status === 'success' ? chalk.green('✓') : chalk.yellow('~');
        const changes = layer.changes || 0;
        console.log(`   ${layerStatus} Layer ${layer.id}: ${layer.name} ${changes > 0 ? chalk.cyan(`(${changes} changes)`) : ''}`);
        
        if (layer.insights && layer.insights.length > 0) {
          layer.insights.forEach(insight => {
            const severity = insight.severity === 'error' ? chalk.red('●') : 
...
function formatSummary(results: any[]) {
  ...
  if (successful.length > 0) {
    ...
    Object.entries(layerStats).forEach(([layerId, stats]: [string, any]) => {
      ...
      console.log(`  Files: ${stats.files}, Changes: ${stats.changes}, Issues: ${stats.issues}`);
    });
  }
}