...
import { NeuroLintOrchestrator } from '../../client/src/lib/neurolint/orchestrator';
...
router.get('/api/layers', (req: Request, res: Response) => {
  const layers = [
    ...
    },
    {
      ...
      description: "Optimizes Next.js App Router patterns, 'use client' directives, and import order.",
      timeout: 30000,
      enabled: true
    },
    {
      ...
      description: "Adds error handling, performance optimizations, and code quality improvements.",
      timeout: 30000,
      enabled: true
    }
  ];
...
router.post('/api/validate-config', (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(layers)) {
      ...
      const invalidLayers = layers.filter(l => !Number.isInteger(l) || l < 1 || l > 6);
      if (invalidLayers.length > 0) {
        errors.push(`Invalid layer IDs: ${invalidLayers.join(', ')}. Must be between 1-6.`);
      }
    }
