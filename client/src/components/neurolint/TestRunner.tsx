...
export function TestRunner() {
  ...
  const keyLayerResults = useMemo(() => {
    const layerStats = keyLayers.map(layerId => {
      const layerTests = results.filter(r => r.testCase.category === getCategoryForLayer(layerId));
      const passed = layerTests.filter(r => r.passed).length;
      const total = layerTests.length;
      const avgTime = total > 0 ? layerTests.reduce((sum, r) => sum + r.executionTime, 0) / total : 0;
  ...
  const performanceInsights = useMemo(() => {
    ...
    if (avgExecutionTime > 100) {
      insights.push("Consider optimizing transformation algorithms - average execution time is above 100ms");
    }

    const failedTests = results.filter(r => !r.passed);
    if (failedTests.length > 0) {
      const commonFailures = failedTests.reduce((acc, test) => {
      ...
      const mostCommonFailure = Object.entries(commonFailures).sort(([,a], [,b]) => b - a)[0];
      if (mostCommonFailure) {
        insights.push(`Most common failure: "${mostCommonFailure[0]}" (${mostCommonFailure[1]} occurrences)`);
      }
    }

    const keyLayerEnabled = enabledLayers.some(id => keyLayers.includes(id));
    if (!keyLayerEnabled) {
      insights.push("Enable config, pattern, or hydration layers for comprehensive testing");
    }
            ...
            return (
              <Card key={layer.layerId} className="relative overflow-hidden">
                <div className={`absolute top-0 left-0 right-0 h-1 ${
                  layer.passRate === 100 ? 'bg-green-500' :
                  layer.passRate >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                }`} />
                ...
                <CardContent className="pt-0">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge variant={layer.passRate === 100 ? "default" : layer.passRate >= 50 ? "secondary" : "destructive"}>
                        {layer.passed}/{layer.total} ({layer.passRate.toFixed(0)}%)
                      </Badge>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-muted-foreground">Avg Time</span>
                      <span className="text-xs font-mono">{layer.avgTime}ms</span>
                    </div>
        ...
        <Tabs defaultValue="all" className="space-y-4">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="config" className="flex items-center gap-1">
              ...
              Config ({results.filter(r => r.testCase.category === 'config').length})
            </TabsTrigger>
            <TabsTrigger value="pattern" className="flex items-center gap-1">
              <Wrench className="w-3 h-3" />
              Pattern ({results.filter(r => r.testCase.category === 'pattern').length})
            </TabsTrigger>
            <TabsTrigger value="hydration" className="flex items-center gap-1">
              <ShieldCheck className="w-3 h-3" />
              Hydration ({results.filter(r => r.testCase.category === 'hydration').length})
            </TabsTrigger>
          </TabsList>
              ...
              <TabsContent key={category} value={category} className="space-y-4">
                <div className="grid gap-4">
                  {filteredResults.map((result, index) => {
                    const isKeyLayer = keyLayers.includes(
                      getCategoryForLayer(result.testCase.category === 'config' ? 1 : result.testCase.category === 'pattern' ? 2 : 4)
                    );
...
function getCategoryColor(category: string): string {
  switch(category) {
    case 'config': return 'border-blue-500 text-blue-700';
    case 'pattern': return 'border-purple-500 text-purple-700';
    case 'hydration': return 'border-green-500 text-green-700';
    case 'component': return 'border-orange-500 text-orange-700';
